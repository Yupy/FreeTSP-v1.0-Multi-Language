...
if (user::$current)
{
    ...
    echo("In <a class='altlink' href='userdetails.php?id=" . user::$current['id'] . "'>your</a> particular case, ");

    if (isset($wait))
    {
        $byboth = $byratio && $byul;
        echo(($byboth ? 'both ' : '').($byratio ? 'your ratio of '.format_ratio(user::$current['uploaded'], user::$current['downloaded']) : '').($byboth ? ' and ' : '').($byul ? 'your total uploaded of '.round($gigs, 2).' GB' : '').' impl'.($byboth ? 'y' : 'ies').' a delay of $wait hours.'.($byboth ? '' : ' (Even if your '.($byratio ? 'total uploaded is '.round($gigs, 2).' GB' : 'ratio is '.format_ratio(user::$current['uploaded'], user::$current['downloaded'])).'.)'));
    }
    else
    {
