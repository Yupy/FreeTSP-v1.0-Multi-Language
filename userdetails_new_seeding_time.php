...
function snatch_table($res)
{
    ...
    $table1 = "<table class='main' border='1' width='100%' cellspacing='0' cellpadding='5'>
                <tr>
                    <td class='rowhead' bgcolor='#FFFFFF' align='center' colspan='9'>
                        <font color='#47B247'>Both Seeding Time And Ratio Achieved</font><br />
                        <font color='#2E5CE6'>Seeding Time Only Achieved</font><br />
                        <font color='#A34747'>Ratio Only Achieved</font><br />
                        <font color='#FF8533'>Keep Seeding</font>
                    </td>
                </tr>
                <tr>
    ...
    while ($arr = mysql_fetch_assoc($res))
    {
        ...
        //-- Start Seeding Color Background --//

        //-- Seeding Time = 72 hrs ( 259200 Secs ) --//
        //-- Ratio = 1.1 ( File Size + 10% ) --//

        if ($arr["seedtime"] >= 259200  && $arr["uploaded"] >= ($arr["size"] * 1.1))
        {
            $seed_color ='#52CC52';
        }

        elseif ($arr["seedtime"] >= 259200 )
        {
            $seed_color ='#2E5CE6';
        }

        elseif ($arr["uploaded"] >= ($arr["size"] * 1.1))
        {
            $seed_color ='#A34747';
        }
        ...
        //-- Finish Seeding Color Background --//

        $table1 .= "<tr>
            <td class='rowhead' bgcolor='$seed_color' align='center'>
            ...
            <td class='rowhead' bgcolor='$seed_color' align='center'>$ratio<br />".($arr['seeder'] == 'yes' ? "
...
if (get_user_class() >= UC_MODERATOR || $user["id"] == $CURUSER["id"])
{
    ...
    //-- Start Snatched Status Expanding Table --//
