...
class ip {
	...
	public static function valid_ip($ip) {
		...
		$key = 'valid::ip::' . sha1($ip);
        if (($valid_ip = $Memcache->get_value($key)) === false) {
			$valid_ip = 1;
			$addr = self::type($ip, $type);
			if ($type === self::IP4) {
				$long_ip = ip2long($ip);
...
//					'2001:3C00::/22',	// RESERVED		11 Jun 04	[3]
			...
			$Memcache->cache_value($key, $valid_ip, self::CACHE_TIME);
		}
	...
	public static function ip_port($in, &$ip, &$port, &$type, &$addr) {
        ...
        if (($ip_port = $Memcache->get_value($key)) === false) {
			...
			if (!$taddr) {
				$Memcache->cache_value($key, 0, self::CACHE_TIME);
				return false;
			}
			...
			$Memcache->cache_value($key, $ip_port, self::CACHE_TIME);
		}
		elseif (!$ip_port)
			return false;
