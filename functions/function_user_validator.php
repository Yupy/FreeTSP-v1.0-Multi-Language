...
function validator($context)
{
    ...
    $hash      = hash_hmac("sha1", user::$current['secret'], $context . $timestamp);
...
function validate($validator, $context, $seconds = 0)
{
    ...
    if ($seconds && vars::$timestamp > $timestamp + $seconds)
    {
        return false;
    }

    $hash = substr(hash_hmac("sha1", user::$current['secret'], $context . $timestamp), 0, 20);
