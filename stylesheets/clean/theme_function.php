...
$lang = array_merge(load_language('style_clean_theme'));
...
function StatusBar()
{
	...
	if (($statusbar_stats = $Memcache->get_value($stats_key)) === false) {
        $get_stats = $db->query('SELECT username, uploaded, downloaded, invites FROM users WHERE id = ' . user::$current['id']);
	...
	{
		$IsDonor = "<img src='{$image_dir}star.png' width='15' height='16' border='0' alt='{$lang['gbl_img_alt_donor']}' title='{$lang['gbl_img_alt_donor']}' />";
	}

	$warn = '';

	if (user::$current['warned'] == "yes")
	{
		$warn = "<img src='{$image_dir}warned.png' width='15' height='16' border='0' alt='{$lang['gbl_img_alt_warned']}' title='{$lang['gbl_img_alt_warned']}' />";
	}
    ...
    {
		$StatusBar = "<div id='statusbar'><p class='home'>{$lang['gbl_table_welcome']}<a class='altlink_user' href='userdetails.php?id=" . user::$current['id'] . "'><font color='#" . get_user_class_color(user::$current['class']) . "'> " . security::html_safe($statusbar_stats['username']) . "</font></a>&nbsp;$IsDonor&nbsp;$warn&nbsp;$usrclass<br />{$lang['gbl_table_rep']}: " . user::$current['reputation'] . "<br />{$lang['gbl_table_invites']}: <a href='invite.php'>" . security::html_safe($statusbar_stats['invites']) . "</a></p><p>";
	}
	else
	{
		$StatusBar = "<div id='statusbar'><p class='home'>{$lang['gbl_table_welcome']}<a class='altlink_user' href='userdetails.php?id=" . user::$current['id'] . "'><font color='#" . get_user_class_color(user::$current['class']) . "'> " . security::html_safe($statusbar_stats['username']) . "</font></a>&nbsp;$IsDonor&nbsp;$warn&nbsp;$usrclass<br />{$lang['gbl_table_invites']}: <a href='invite.php'>" . security::html_safe($statusbar_stats['invites']) . "</a></p><p>";
	}

    //-- Finish Temp Demote By Retro 3 of 3 --//
	$StatusBar .= "</p><p class='home'>{$lang['gbl_table_ratio']}:$ratio<br />&nbsp;&nbsp;{$lang['gbl_table_uploaded']}:$upped<br />&nbsp;&nbsp;{$lang['gbl_table_downloaded']}:$downed<br />&nbsp;&nbsp;{$lang['gbl_table_active']}:&nbsp;<img alt='{$lang['gbl_img_alt_seeding']}' title='{$lang['gbl_img_alt_seeding']}' src='{$image_dir}up.png' height='12' width='12'/>&nbsp;{$seedleech['yes']}&nbsp;&nbsp;<img alt='{$lang['gbl_img_alt_leeching']}' title='{$lang['gbl_img_alt_leeching']}' src='{$image_dir}dl.png' height='12' width='12'/>&nbsp;{$seedleech['no']}</p><div align='center'>
