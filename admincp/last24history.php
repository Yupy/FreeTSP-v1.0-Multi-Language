...
$lang = array_merge(load_language('adm_last24history'),
...
site_header("{$lang['title_last24']}", false);

begin_frame("{$lang['title_last24']}", 'center');
...
switch ($do)
{
    case 'showgrap':
        ...
        $title = new title($lang['text_history_for'] . (isset($log['year']) ? 'year ' . $log['year'] : '') . (isset($log['month']) ? $lang['text_month'] . $log['month'] . $lang['text_year'] . $var['year'] : ''));
        ...
        $line->set_key($lang['text_visits'], 12);
    ...
    default:
        ...
        if (count($log))
        {
            foreach ($log
                    ...
                    $year => $more)
            {
                echo("<fieldset><legend><a href='controlpanel.php?fileaction=4&amp;do=showgrap&amp;year=$year'>{$lang['text_year']} $year</a></legend>");
