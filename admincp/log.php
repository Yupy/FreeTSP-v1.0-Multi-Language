...
$lang = array_merge(load_language('adm_log'));
...
site_header("{$lang['title_log']}", false);
...
echo("<h1>{$lang['title_log']}</h1>\n");

if ($res->num_rows == 0)
{
    echo("<strong>{$lang['text_empty']}</strong>");
}
else
{
    echo $menu;
    ...
    echo("<tr>
            <td class='colhead' align='center' width='60px'>{$lang['text_date']}</td>
            <td class='colhead' align='center' width='60px'>{$lang['text_time']}</td>
            <td class='colhead' align='left' width='450px'>{$lang['text_event']}</td>
        </tr>");
